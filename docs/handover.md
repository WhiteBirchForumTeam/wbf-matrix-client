...
> 給下一個接手的人（人或 agent）。2026-09-06 寫、2026-09-08 更新，每次交接更新。設計理由不在這裡，在 `docs/design/`；這裡只講**現況、怎麼跑、坑、下一步**。
...
## 7. 下一步（維護者 2026-09-06 同意的順序，2026-09-08 更新）

做完的（都合併了）：vault 與金鑰（#11）、`cache.db` 多帳號混存（#13；rusqlite 0.40 與 matrix-sdk 合得來，代價是 Windows 要 Strawberry Perl，local-cache-db §3）、媒體儲存池（#14）、`recent` 改成拉窗＋`Event/Batch` 串流與三層分工 `RecentPlan { max_events, window, batch }`（#16，issue #15）。

還沒做的：

0. ⚠️ **房間金鑰備份**（維護者 2026-09-09 提，排到最前面）：設計定案在 local-cache-db §10 與 CLI 規格 §3.6／§10，實作分兩個 PR ——先 **conf 檔**（CLI 規格 §10；它是「備份預設開、可手動關」的前提），再做 **金鑰備份**（server 端標準 backup、本地 `room-keys/` 加密池、`key-backup` 命令）。
...
4. 串流／seek 對著媒體池讀（local-cache-db §8.6）：`seek` 命令現在仍直接打 server。
5. ⚠️ UI 落地前要確認「進房逐房翻頁」真的存在：`recent` 被 `max_events` 停下時，`[last_ls, 舊水位)` 那段是永久洞，只有逐房 `/messages` 會補（PR #16 審查記錄）。
