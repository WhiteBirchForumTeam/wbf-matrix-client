...
> 兩支 PR：**A**（探活不帶 token、以 server 為鍵；四條線；登出封鎖）、**B**（wbf 帳號不建 matrix-sdk 的 Client：登入登出走標準 HTTP、`m/` 由 OlmEngine 開、房間與送訊息走 WS、備份暫擋）。
...
## 6. wbf 帳號暫時做不到的（PR B 之後、E2EE 與備份搬家之前）

| 功能 | 現況（走 Client） | 不建 Client 之後 |
|---|---|---|
| `room.list`／`room.get` 的 `server`／`both` | Client 的 /sync | 橋 `JoinedRooms`（0x13/0x28）＋每房 `GetState`（0x14/0x21）組 `Conversation`，寫進 `room_list`；`local` 不變 |
| `room.send_text` | `Room::send`（含加密） | `Event/Send` 明文；**加密房拒絕**（明確的錯），E2EE 那支接 `encrypt_and_send` |
| `room.send_file` 的送事件半段 | `Room::send` | `Event/Send` 帶 `attachments`（約定 §5.2 的宣告終於成立） |
| `room.history` 錨點不在本地 | `/context` | 拒絕（1100），等 wbfuwunel #64 的 `before_event_id` |
| `watch`（CLI） | /sync 的迴圈 | 拒絕：daemon 的新訊息走訂閱＋推播（第 6 階段） |
| `backup.*`、`recovery.*` | Client 的備份與 SSSS | **拒絕、回明確的錯**（🚫 不靜默失效）；搬到 crypto 層＋橋（`BackupMachine`、`SecretStorageKey`、橋的 `/room_keys`、account data）排在 E2EE 的 RPC 面之後 |
| 登出閘門的「server 那份救得回來嗎」 | Client 問 backup status | 問不到 → 只認本機的 recovery key 或 `accept_history_loss`（fail closed） |
