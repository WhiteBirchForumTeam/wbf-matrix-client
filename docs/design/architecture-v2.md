...
> ✅ **2026-09-13：獨佔真的實作了**（`crates/wbf-daemon/src/lock.rs`）。起來時對
> `<data dir>/daemon.lock` 拿一把 OS 層排他鎖（`std::fs::File::try_lock`），**拿不到就不啟動**，
...
### 6.1 daemon 跟 server 開**四條** WS，一條一個用途（維護者 2026-09-12 定）
...
| **媒體** | 上傳與下載（`Upload/*`、`Download/*`、`Stream/*`） | 資料平面，長時間高頻寫。⭐ 它是最會把佇列塞爆的那個，所以它**只能塞爆自己** |
