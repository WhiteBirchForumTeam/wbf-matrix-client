...
- `WsLink::start` 帶一個 `ReceivedHook`，預設是空函數。讀取 task 每 decode 一個 pack、**查完表之後**叫它一次，不管有沒有人在等；
  無主的也叫，`route = Unmatched`。
...
- 送出 task 寫 sink 失敗 → 關閉佇列；之後每個 `send` 立刻回 `Network`。讀取 task 會因為 socket 也死了而結束，走 §7 的關線。
...
讀取 task 結束的三種原因：對方 `Close`、socket 錯、連續 `CORRUPT_FRAME_BUDGET` 個 frame 解不開（跟 server 的 `wbf_ws_corrupt_budget` 同一個想法：
一個壞 frame 不值得斷線，一連串就是這條線只剩壞的）。結束後：

1. `SessionTable::fail_all(reason)`：每一項 `fail(Network(reason))`，在等的請求全部回錯、串流與訂閱的 channel 關掉。
2. 送出佇列關掉，之後 `send` 立刻回錯。
3. `WsLink::is_closed()` 變 true。重連不在這層。
