...
### 5.4 🚨 一條廣播裝所有事件 = 進度會把訊息擠掉（2026-09-13 自我審查補的）

`CoreEvent` 現在全部走**同一條** `broadcast`（深度 256）。一個 2 GB 的上傳每塊發一則
`Progress`，幾秒鐘就能把佇列翻過一輪 —— 於是**同一時間進來的 `room.message` 被擠掉**。

🚨 這正是 architecture-v2 §6.1.1 給上游連線立的那條規矩（**「誰會塞爆佇列」×「掉了救不救得回」**）
—— 而我們自己的事件匯流排違反了它。⭐ 對上游成立的道理，對內部沒有理由不成立。

兩層處置，都要：

1. **在源頭節流 `Progress`**：同一個 job **每 100 ms 或每 1% 才發一則**，最後一則（完成）一定發。
   ⭐ 進度的用途是畫一根進度條，🚫 沒有人需要每 64 KiB 一則。
   📎 這同時省掉 RPC 上的流量 —— 一個大檔本來會推出上萬則。
2. **把「掉了救不回」的跟「掉了無所謂」的分開**：`Message`／`SyncState` 走一條、
   `Note`／`Progress` 走另一條。⚠️ 這是實作階段 2／4 要一起做的結構決定，
   🚫 不要等到「使用者說上傳時漏訊息」才回頭改。

### 5.5 HTTP 那條路也要寫進 `cache.db`（2026-09-13 自我審查補的）
...
| 3 | 所有事件一條廣播 | 🚨 進度會把訊息**擠掉**，違反我們自己對上游立的規矩 | 源頭節流 ＋ 兩條佇列分開（§5.4） |
| 4 | HTTP 帳號的事件在 matrix-sdk store | 🚨 那 `sync: "local"` 對它們是**空的** | HTTP 那條也鏡射進 `cache.db`（§5.5） |

⭐ 四個裡有三個是**「文件自己前後矛盾」**，而不是「想得不夠遠」——
📎 值得記住的教訓：**把兩條規則寫在同一份文件的不同章節，不代表它們相容**。
