...
| 5 | `cancel`（§9） | ❌ |
| 6 | sdk 的 `Event/Subscribe`（`0x04`）／`Unsubscribe`（`0x05`）／`Push`（`0x06`） | ✅ 2026-09-22：codec 對著 server 向量、`WbfClient::room_subscription`／core `room_sync.rs`（`design/room-sync.md`：讀水位 → 訂閱 → 補窗 job → 背景收推播，gap 再補；🚫 還沒接 RPC） |
