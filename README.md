...
| [`local-cache-db.md`](docs/design/local-cache-db.md) | 本地資料庫：加密的暫存快取、主金鑰與 local password、與 matrix-sdk store 的分工。§4 vault 做了（`wbf-sdk::vault`），`cache.db` 還沒 |
