...
pub async fn fetch<C: PackChannel>(
    ...
    if entry.complete {
        if let Some(pool_file) = &entry.pool_file {
            // DB 說有：檔也要真的在（§8.3「兩邊都問」）；不在就當沒快取，reset 後重下。
            if pool.open_read(pool_file).is_ok() {
...
/// 啟動時掃一次（§8.5 最後一條）：DB 說完整但檔不在 → reset；`pending/` 裡沒有對應半成品列的 → 刪檔；
/// 半成品超過保護期沒人續 → 刪檔加 reset。
///
/// Return:
///     Ok((reset_rows, removed_pending))
pub fn sweep(
    cache: &mut Cache,
    pool: &MediaPool,
    protect: Duration,
    now_millis: i64,
) -> Result<(u64, u64), SdkError> {
