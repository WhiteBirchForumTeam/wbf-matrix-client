...
/// 而 Windows 上（看 ACL、不看模式位元）卻全綠。⭐ 這正是「綠燈不代表沒壞」的那種洞
/// （PR #31 審查 cirno🔴 —— 我在 Windows 上跑，所以自己看不到）。
///
/// 📎 用 `wbf_sdk::vault::write_private`：跟 `local.key` 同一支，**建立時**就是 0600
/// （🚫 不是先寫再 chmod —— 那中間有一段別人讀得到的時間）。
fn write_token_like_a_frontend(path: &Path) {
    wbf_sdk::vault::write_private(path, &TOKEN).expect("寫 token 檔");
}

fn spawn_daemon(data_dir: &Path, token_file: &Path) -> (Child, Ready) {
    // ⚠️ 先確認**測試自己**把 token 檔弄對了：不然 Unix 上 daemon 會 fail closed 地拒絕啟動，
    // 而我們只會看到「ready 那行不是 JSON」這種查不出原因的 panic。
    assert!(
        wbf_daemon::token::is_private(token_file).expect("token 檔在"),
        "測試要先把 token 檔寫成私有的（write_token_like_a_frontend）"
    );
    let mut child = Command::new(env!("CARGO_BIN_EXE_wbf-matrix-client-daemon"))
        ...
        .arg(token_file)
        .stdout(Stdio::piped())
        // ⚠️ 留著 stderr：起不來的時候，原因只在那裡。🚫 不要丟到 null 然後對著空的 stdout 猜。
        .stderr(Stdio::piped())
        .spawn()
    ...
    BufReader::new(child.stdout.as_mut().expect("piped stdout"))
        .read_line(&mut line)
        .expect("讀 daemon 的 stdout");
    if line.trim().is_empty() {
        // 沒說 ready 就是根本沒起來 —— 把它的抱怨原樣印出來。
        panic!(
            "the daemon exited before saying ready: {}",
            complaint_of(child)
        );
    }
...
async fn the_daemon_process_announces_ready_leaves_the_token_alone_and_cleans_up_on_shutdown() {
    ...
    write_token_like_a_frontend(&token_file);
...
/// 起不來的時候它到底說了什麼。
fn complaint_of(mut child: Child) -> String {
    let _ = child.kill();
    match child.wait_with_output() {
        Ok(output) => String::from_utf8_lossy(&output.stderr).trim().to_string(),
        Err(error) => format!("(stderr 讀不到：{error})"),
    }
}
...
#[test]
fn a_token_file_of_the_wrong_length_stops_the_daemon_and_says_so() {
    let dir = tempfile::tempdir().unwrap();
    let token_file = dir.path().join("daemon.token");
    wbf_sdk::vault::write_private(&token_file, &[7u8; 255]).unwrap();
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_wbf-matrix-client-daemon"))
        ...
        .output()
        .expect("the daemon binary runs");
    assert!(!output.status.success(), "255 byte 的 token 不該啟動得起來");
    assert!(output.stdout.is_empty(), "🚫 不該印 ready");
    let complaint = String::from_utf8_lossy(&output.stderr);
    assert!(complaint.contains("255 bytes"), "{complaint}");
    assert!(complaint.contains("256"), "要說出正確的長度：{complaint}");
    // 起不來就🚫 不該在那個目錄留下東西。
    assert!(!dir.path().join("daemon.json").exists());
}
