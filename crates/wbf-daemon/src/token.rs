...
//! ```
//!
//! ⚠️ **抹掉的是前端**，不是 daemon：daemon 不知道前端有沒有拿到 ready、也可能有別的前端正在
//! 起來。daemon 這一側的責任只有兩件：讀完就把 bytes 丟掉（只留導出的金鑰），以及**之後永遠
//! 不再回頭讀那個路徑**——所以第 4 步刪掉它不會讓任何東西壞掉（`loopback.rs` 有一條測這個）。
//!
